...
class WorkItem:
    def __init__(self ,url:str, proxy:str = None, request:int = 0):
        self.url = url
        self.proxy = proxy
...
async def process_page(worker_id: int,work_item: WorkItem, queue: Queue, session: CloudflareScraper):
    ...
    if (200 <= status <= 302) or (status >= 500):
        ...
        if (workItemCounter:= work_item.request_count+1 ) < MAX_REQUESTS_TO_SITE:
            queue.put_nowait( WorkItem(work_item.url, work_item.proxy ,workItemCounter) )
            return
    elif work_item.url:
        if work_item.request_count>0:
            logger.info(f'[{worker_id}] {work_item.url} - {status}, proxy: {work_item.proxy} ({work_item.request_count})')
        ...
        if not work_item.proxy:
            proxy_list = proxies.getAll()
            for proxy in proxy_list:
                queue.put_nowait( WorkItem(work_item.url, f'http://{proxy}') )
            return
    
    while not (url := sites.getNext()):
        await asyncio.sleep(0)
        # return

    queue.put_nowait( WorkItem(url, None) )
